...
var datepicker = {
	...
	init: function init(inputElem, options) {
		...
		this.inputElem.addEventListener('focus', event => this.updateCalendar(null));
		
		this.inputElem.addEventListener('blur', event => {
			if(this.options.hideCalendarOnClickOff) 
				setTimeout((event) => this.closeCalendar(event), 50);
		});
		this.container.addEventListener('blur', event => this.closeCalendar(event));
		this.container.addEventListener('focus', event => this.focusFlag = true );
		
		this.inputElem.addEventListener('change', event => {
			if(this.inputElem.value) {
				if(!this.dateRegex.test(this.inputElem.value) || new Date(this.inputElem.value) < this.options.minDate || new Date(this.inputElem.value) > this.options.maxDate) {
		...
		this.inputElem.addEventListener('input', event => {
			let parts = this.inputElem.value.split('/');
			let guess = this.options.defaultDate;
	...
	generateMonthHtml: function generateMonthHtml(dates, selectedDate) {
		...
		calendar.classList.add('calendar');
			...
			calendar.appendChild(calendarHeader);
				...
				prevMonthBtn.addEventListener('click', event =>  this.updateCalendar(prevMonth) );
				...
				currMonthLabel.addEventListener('click', event => {
					this.changeViewMode('month', dates[15].date);
					this.viewMode = 'month';
				});
				...
				nextMonthBtn.addEventListener('click', event => this.updateCalendar(nextMonth) );
			...
			calendar.appendChild(calendarBody);
				...
				for(let i = 0; i < dates.length; i++) {
					...
					if(selectedDate && this.utils.toISODateFormat(selectedDate) == this.utils.toISODateFormat(dates[i].date)) calendarDate.classList.add('selected-date');
					calendarDate.classList.add((dates[i].currentMonth ? 'current-month' : 'not-current-month' ));
					calendarDate.title = this.utils.toNormalDateFormat(dates[i].date);
					calendarDate.setAttribute('data-date-value', this.utils.toISODateFormat(dates[i].date));
					
					calendarDate.innerText = dates[i].date.getDate();
					if(this.options.minDate && dates[i].date < this.options.minDate ) {
						calendarDate.classList.add('calendar-item-disabled');
					}
					if(this.options.maxDate && dates[i].date > this.options.maxDate ) {
						calendarDate.classList.add('calendar-item-disabled');
					}
					
					if(!calendarDate.classList.contains('calendar-item-disabled')) {
						calendarDate.addEventListener('click', event => this.pickDate(event, new Date(dates[i].date), false));
	...
	generateYearHtml: function generateYearHtml(dates, selectedDate) {
		...
		calendar.classList.add('calendar');
			...
			calendar.appendChild(calendarHeader);
				...
				prevYearBtn.addEventListener('click', event =>  this.updateCalendar(prevYear) );
				...
				currYearLabel.addEventListener('click', event => {
					this.changeViewMode('year', dates[0].date);
				});
				...
				nextYearBtn.addEventListener('click', event => this.updateCalendar(nextYear) );
			...
			calendar.appendChild(calendarBody);
				...
				for(let i = 0; i < dates.length; i++) {
					...
					if(selectedDate && this.utils.toISODateFormat(this.utils.getFirstDayOfTheMonth(selectedDate)) == this.utils.toISODateFormat(this.utils.getFirstDayOfTheMonth(dates[i].date))) calendarMonth.classList.add('selected-date');
					calendarMonth.title = this.utils.getMonthName(dates[i].date) + ' ' + dates[i].date.getFullYear();
					calendarMonth.setAttribute('data-date-value', this.utils.toISODateFormat(dates[i].date));
					calendarMonth.innerText = this.utils.getMonthName(dates[i].date).slice(0, 3);
					let minMonth = this.utils.getFirstDayOfTheMonth(this.options.minDate);
					if(this.options.minDate && dates[i].date < minMonth ) {
	...
	generateDecadeHtml: function generateDecadeHtml(dates, selectedDate) {
		...
		calendar.classList.add('calendar');
			...
			calendar.appendChild(calendarHeader);
				...
				prevDecadeBtn.addEventListener('click', event =>  this.updateCalendar(prevDecade) );
				...
				nextDecadeBtn.addEventListener('click', event => this.updateCalendar(nextDecade) );
			...
			calendar.appendChild(calendarBody);
				...
				for(let i = 0; i < dates.length; i++) {
					...
					if(selectedDate && selectedDate.getFullYear() == dates[i].date.getFullYear()) calendarYear.classList.add('selected-date');
					calendarYear.title = dates[i].date.getFullYear();
					calendarYear.setAttribute('data-date-value', this.utils.toISODateFormat(dates[i].date));
					calendarYear.innerText = dates[i].date.getFullYear();
					
					let minYear = this.options.minDate.getFullYear();
					if(this.options.minDate && dates[i].date.getFullYear() < minYear ) {
						calendarYear.classList.add('calendar-item-disabled');
					}
					let maxYear = this.options.maxDate.getFullYear();
					if(this.options.maxDate && dates[i].date.getFullYear() > maxYear ) {
						calendarYear.classList.add('calendar-item-disabled');
					}
					
					if(!calendarYear.classList.contains('calendar-item-disabled')) {
						calendarYear.addEventListener('click', event => {
							this.changeViewMode('month', dates[i].date);
						});
	...
	generateTodayButton: function generateTodayButton() {
		...
		todayButton.addEventListener('click', event => {
			this.viewMode = 'day';	// Maybe change this to check min view mode
			this.pickDate(event, new Date(), true);
		});
		return todayButton;
	},
